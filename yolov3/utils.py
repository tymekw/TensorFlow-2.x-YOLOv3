...
def draw_bboxv2(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):
    NUM_CLASS = read_class_names(CLASSES)
    num_classes = len(NUM_CLASS)
    image_h, image_w, _ = image.shape
    #hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    ...
    #colors = list(map(lambda x: (int(x[0] * 255), int(x[1] * 255), int(x[2] * 255)), colors))
    #BeGieEr XDDDDDD
    colors = {"car":(255, 0, 0), "bus":(0, 0, 255), "truck":(0, 0, 255), "bicycle":(0, 255, 0), "motorbike":(0, 255, 0), "train":(0,255, 255)}
    #random.seed(0)
    #random.shuffle(colors)
    #random.seed(None)
    ...
    for i, bbox in enumerate(bboxes):
        ...
        score = bbox[4]
        class_ind = int(bbox[5])
        bbox_color = rectangle_colors if rectangle_colors != '' else colors[NUM_CLASS[class_ind]]
        bbox_thick = int(0.6 * (image_h + image_w) / 1000)
        if bbox_thick < 1: bbox_thick = 1
        fontScale = 0.75 * bbox_thick
        (x1, y1), (x2, y2) = (coor[0], coor[1]), (coor[2], coor[3])
